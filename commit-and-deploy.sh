...
if [[ -x ./deploy-prod.sh ]]; then
  read -p "Run ./deploy-prod.sh now? [y/N] " deploy_confirm
  if [[ "$deploy_confirm" == "y" || "$deploy_confirm" == "Y" ]]; then
    ./deploy-prod.sh
  else
    echo "Deployment skipped. Run ./deploy-prod.sh when ready."
  fi
else
  echo "deploy-prod.sh not found or not executable."
  echo "Use ./deploy-local-direct.sh or ./deploy-extract-and-build.sh if appropriate."
fi
