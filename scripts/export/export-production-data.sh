...
download_via_ssh() {
    ...
    ssh -q root@${SERVER_DOMAIN} "cd $SERVER_PATH && php artisan export:customers" 2>/dev/null || {
    ...
    AUTH_TOKEN=$(ssh -q root@${SERVER_DOMAIN} "cd $SERVER_PATH && grep ADMIN_API_TOKEN .env | cut -d= -f2" 2>/dev/null || echo "")
...
backup_database() {
    ...
    DB_NAME=$(ssh -q root@${SERVER_DOMAIN} "cd $SERVER_PATH && grep DB_DATABASE .env | cut -d= -f2" 2>/dev/null || echo "insurance2026")
    DB_USER=$(ssh -q root@${SERVER_DOMAIN} "cd $SERVER_PATH && grep DB_USERNAME .env | cut -d= -f2" 2>/dev/null || echo "root")
    DB_PASS=$(ssh -q root@${SERVER_DOMAIN} "cd $SERVER_PATH && grep DB_PASSWORD .env | cut -d= -f2" 2>/dev/null || echo "")
    DB_HOST=$(ssh -q root@${SERVER_DOMAIN} "cd $SERVER_PATH && grep DB_HOST .env | cut -d= -f2" 2>/dev/null || echo "localhost")
...
# Then download:
scp root@${SERVER_DOMAIN}:/home/tamserve/insurance2026/storage/exports/*.csv ./downloads/
EOF
    echo ""
}
