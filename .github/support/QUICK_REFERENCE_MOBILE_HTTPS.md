...
```
┌─ User: "I can't access watheeq.plus on my phone"
│
├─ Q1: Error message?
...
curl -sI https://watheeq.plus | head -1

# Test 2: Certificate valid?
openssl s_client -connect watheeq.plus:443 -servername watheeq.plus </dev/null 2>/dev/null | grep -E "(subject=|notAfter=)"

# Test 3: Reverb working?
curl -s https://watheeq.plus/api/customer/ip | head -c 50
```
...
2. Turn it OFF
3. Go to Safari and reload watheeq.plus
4. Does it work now?
...
**Two options:**
1. Keep Private Relay disabled for watheeq.plus
...
2. Press F12 → Console
3. Paste: fetch('https://watheeq.plus').catch(e => console.log(e))
