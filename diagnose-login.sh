...
HEALTH=$(curl -k -s -w "%{http_code}" -o /tmp/health_check.json https://lexusforbon.it.com/api/health)
if [ "$HEALTH" = "200" ]; then
...
curl -k -X POST https://lexusforbon.it.com/api/admin/login \
...
echo "   • Admin user dr@lexusforbon.it.com doesn't exist in database"
...
echo "   3. Run: SELECT * FROM users WHERE email='dr@lexusforbon.it.com';"
