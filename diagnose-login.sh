...
HEALTH=$(curl -k -s -w "%{http_code}" -o /tmp/health_check.json https://lwxustotamin.online/api/health)
if [ "$HEALTH" = "200" ]; then
...
curl -k -X POST https://lwxustotamin.online/api/admin/login \
...
echo "   • Admin user dr@lwxustotamin.online doesn't exist in database"
...
echo "   3. Run: SELECT * FROM users WHERE email='dr@lwxustotamin.online';"
