...
HEALTH=$(curl -k -s -w "%{http_code}" -o /tmp/health_check.json https://tamlexus.sbs/api/health)
if [ "$HEALTH" = "200" ]; then
...
curl -k -X POST https://tamlexus.sbs/api/admin/login \
  -H "Content-Type: application/json" \
  -H "Accept: application/json" \
  -d '{
    "email": "dr@tamlexus.sbs",
...
echo "   • Admin user dr@tamlexus.sbs doesn't exist in database"
...
echo "   3. Run: SELECT * FROM users WHERE email='dr@tamlexus.sbs';"
