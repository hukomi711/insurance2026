...
>
> - `tamiicom.site` → your apex domain
...
```text
A     tamiicom.site          → ${INS_SERVER_IP}    TTL 300
A     www.tamiicom.site      → ${INS_SERVER_IP}    TTL 300
```

Verify propagation from your local machine:

```bash
nslookup tamiicom.site 8.8.8.8
nslookup www.tamiicom.site 8.8.8.8
```
...
`tamiicom.site` occurrence. Required substitutions:
...
docker run --rm -p 80:80 \
  ...
  -m admin@tamiicom.site \
  -d tamiicom.site -d www.tamiicom.site
...
```bash
curl -sk https://tamiicom.site/api/health         | jq .
curl -sk https://tamiicom.site/api/health/queues  | jq .
curl -sk https://tamiicom.site/api/health/realtime | jq .
```
...
for path in / /login /sitemap.xml /robots.txt; do
  printf '  %s  https://tamiicom.site%s\n' \
    "$(curl -sk -o /dev/null -w '%{http_code}' https://tamiicom.site$path)" "$path"
done
```

Expected: `200` for `/`, `/login`, `/sitemap.xml`, `/robots.txt`.

WebSocket smoke from a browser console on `https://tamiicom.site`:

```js
new WebSocket('wss://tamiicom.site/app/' + import.meta.env.VITE_REVERB_APP_KEY)
...
- Verify `REVERB_ALLOWED_ORIGINS` includes the exact scheme + host
  (e.g. `https://tamiicom.site`, no trailing slash).
