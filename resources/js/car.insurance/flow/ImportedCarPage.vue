            ...
            <div class="flex flex-col lg:flex-row w-full">
                ...
                <div class="w-full flex flex-col lg:w-4/6 p-4">
                    ...
                    <form class="space-y-8 mb-8" @submit.prevent="handleSubmit">
                        ...
                        <!-- Row 2: Customs Card Number + Manufacturing Year -->
                        <div class="flex flex-col md:flex-row gap-4 justify-between">
                            ...
                            <div class="w-full md:w-1/2">
                                ...
                                <p v-if="errors.customsCardNumber" class="text-red-500 text-xs mt-1">
                        ...
                        <div class="rounded-lg p-4 border"
                            ...
                            <label class="flex items-start gap-3 cursor-pointer">
                                <input id="imported-accept-consent" v-model="form.acceptConsent" type="checkbox"
                                    class="mt-1 h-4 w-4 accent-blue-600"
                                    :aria-invalid="errors.acceptConsent ? 'true' : 'false'"
                                    aria-describedby="imported-consent-error" />
                            ...
                            <p v-if="errors.acceptConsent" id="imported-consent-error" class="text-red-500 text-xs mt-2">
                                {{ errors.acceptConsent }}
                            </p>
                        </div>

                        <!-- Customs Card Help — Mobile only -->
...
const customsCardSrc = new URL( '../../../images/motorapp/custone_id.webp', import.meta.url ).href;
...
// Manufacturing year options — restricted to last 5 years for imported cars
const currentGregorianYear = new Date().getFullYear();
const manufacturingYearOptions = Array.from( { length: 5 }, ( _, i ) =>
{
...
onMounted( () =>
{
    const saved = sessionStorage.getItem( 'importedCarDetails' );
    if ( saved )
    {
        try
        {
            ...
            form.customsCardNumber = data.customsCardNumber || '';
...
function onCustomsCardInput ( e )
{
    form.customsCardNumber = e.target.value.replace( /\D/g, '' ).slice( 0, 15 );
    clearFormError();
    if ( errors.customsCardNumber ) validateCustomsCard();
}
...
function validateCustomsCard ()
{
    if ( !form.customsCardNumber )
    {
        errors.customsCardNumber = 'رقم البطاقة الجمركية مطلوب';
        return false;
    }
    if ( form.customsCardNumber.length < 8 || form.customsCardNumber.length > 15 )
    {
        errors.customsCardNumber = 'رقم البطاقة الجمركية يجب أن يكون بين 8 و 15 رقم';
        return false;
    }
    errors.customsCardNumber = '';
    return true;
...
function validateManufacturingYear ()
{
    ...
    {
        errors.manufacturingYear = `سنة الصنع يجب أن تكون بين ${ currentYear - 4 } و ${ currentYear }`;
        return false;
    }
    errors.manufacturingYear = '';
    return true;
...
async function handleSubmit ()
{
    ...
    if ( !isIdentityValid || !isBirthValid || !isCustomsCardValid || !isYearValid || !isConsentValid ) {
    ...
    // Save to session
    sessionStorage.setItem( 'importedCarDetails', JSON.stringify( {
        identityNumber: form.identityNumber,
        birthMonth: form.birthMonth,
        birthYear: form.birthYear,
        customsCardNumber: form.customsCardNumber,
