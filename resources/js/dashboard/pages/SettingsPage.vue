<template>
    <div>
        ...
        <div class="admin-settings-tabs mb-6 rounded-xl border p-2">
            <div class="grid grid-cols-1 gap-2 md:grid-cols-3">
                <button
                    v-for="tab in tabItems"
                    :key="tab.key"
                    type="button"
                    class="admin-settings-tab group flex items-center gap-3 rounded-lg border px-4 py-3 text-right transition-all duration-200"
                    :class="{ 'admin-settings-tab--active': activeTab === tab.key }"
                    @click="activeTab = tab.key"
                >
                    <span
                        class="admin-settings-tab-icon flex h-10 w-10 items-center justify-center rounded-lg text-sm transition-all duration-200"
                    >
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div v-if="activeTab === 'general'" class="lg:col-span-2 space-y-6">
                <div id="general-settings" class="admin-settings-card rounded-xl p-4 transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md md:p-5">
                    <button type="button" class="admin-settings-section-trigger mb-5 flex w-full items-center justify-between gap-3 rounded-xl border border-transparent px-1 py-2 text-right transition-colors" @click="generalPanel.base = !generalPanel.base">
                    ...
                    <div v-show="generalPanel.base" class="space-y-4">
                        ...
                        <div>
                            ...
                            <textarea id="siteDescription" v-model="settings.siteDescription" rows="3" name="siteDescription" autocomplete="off" class="w-full rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none resize-none" :style="{ backgroundColor: 'var(--admin-input-bg)', borderWidth: '1px', borderColor: 'var(--admin-input-border)', color: 'var(--admin-input-text)' }"></textarea>
                        </div>
                        <div>
                ...
                <div class="admin-settings-card rounded-xl p-4 transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md md:p-5">
                    <button type="button" class="admin-settings-section-trigger mb-5 flex w-full items-center justify-between gap-3 rounded-xl border border-transparent px-1 py-2 text-right transition-colors" @click="generalPanel.contact = !generalPanel.contact">
                        <div class="flex items-center gap-3">
                    ...
                    <div v-show="generalPanel.contact" class="space-y-4">
                        ...
                        <div>
                            ...
                            <input id="whatsappNumber" v-model="siteSettings.whatsapp_number" type="tel" name="whatsapp_number" autocomplete="tel" dir="ltr" placeholder="0597777777 أو 966597777777" class="w-full rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-[#25D366] focus:border-transparent outline-none text-left" :style="{ backgroundColor: 'var(--admin-input-bg)', borderWidth: '1px', borderColor: 'var(--admin-input-border)', color: 'var(--admin-input-text)' }" />
                        ...
                        <div>
                            ...
                            <textarea id="whatsappMessage" v-model="siteSettings.whatsapp_message" rows="2" name="whatsapp_message" autocomplete="off" placeholder="مرحباً، أرغب في الاستفسار..." class="w-full rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-[#25D366] focus:border-transparent outline-none resize-none" :style="{ backgroundColor: 'var(--admin-input-bg)', borderWidth: '1px', borderColor: 'var(--admin-input-border)', color: 'var(--admin-input-text)' }"></textarea>
                        </div>
                        ...
                        <div>
                            ...
                            <input id="supportPhone" v-model="siteSettings.support_phone" type="tel" name="support_phone" autocomplete="tel" dir="ltr" class="w-full rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none text-left" :style="{ backgroundColor: 'var(--admin-input-bg)', borderWidth: '1px', borderColor: 'var(--admin-input-border)', color: 'var(--admin-input-text)' }" />
                        </div>
                        ...
                        <div>
                            ...
                            <input id="contactEmailPublic" v-model="siteSettings.contact_email" type="email" name="contact_email" autocomplete="email" dir="ltr" class="w-full rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none text-left" :style="{ backgroundColor: 'var(--admin-input-bg)', borderWidth: '1px', borderColor: 'var(--admin-input-border)', color: 'var(--admin-input-text)' }" />
                        </div>
                ...
                <div class="admin-settings-card rounded-2xl p-4 transition-all duration-200 hover:-translate-y-0.5 hover:shadow-lg md:p-5">
                    <button type="button" class="admin-settings-section-trigger mb-5 flex w-full items-center justify-between gap-3 rounded-xl border border-transparent px-1 py-2 text-right transition-colors" @click="generalPanel.notifications = !generalPanel.notifications">
                        <div class="flex items-center gap-3">
            ...
            <div v-else-if="activeTab === 'account'" class="lg:col-span-2 space-y-6">
                <div id="account-settings" class="admin-settings-card rounded-xl p-4 transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md md:p-5">
                    ...
                    <div class="admin-settings-muted-panel flex items-center gap-3 mb-4 rounded-xl border border-dashed p-4">
                    ...
                    <div class="admin-settings-muted-panel rounded-xl border border-dashed p-4 text-sm">
            ...
            <div v-else class="lg:col-span-2 space-y-6">
                <div id="password-settings" class="admin-settings-card rounded-xl p-4 transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md md:p-5">
                    <div class="mb-5 flex items-center gap-3">
                    ...
                    <form class="space-y-4" @submit.prevent="changePassword">
                        ...
                        <button type="submit" class="admin-primary-button text-sm font-medium px-5 py-2.5 rounded-xl transition-colors">تغيير كلمة المرور</button>
                    </form>
                </div>
            </div>

            <div class="space-y-6">
                <div class="admin-settings-card rounded-xl p-5 transition-colors duration-200">
                ...
                <div id="account-settings-sidebar" class="admin-settings-card rounded-xl p-5 transition-colors duration-200">
                ...
                <button class="admin-primary-button w-full font-medium py-3 rounded-xl transition-colors" @click="saveSettings">
...
.admin-settings-section-trigger {
    color: var(--admin-text);
    background: transparent;
}

.admin-settings-section-trigger:hover {
    background: var(--admin-hover);
    border-color: var(--admin-card-border);
}

.admin-settings-muted-panel {
    background: var(--admin-surface-2);
    border-color: var(--admin-card-border);
...
.admin-primary-button:hover { filter: brightness(0.94); }
</style>
